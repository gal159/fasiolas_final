...
async function sendResetEmail(toEmail: string, resetLink: string): Promise<void> {
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      html: [
        '<div style="margin:0;padding:32px 16px;background:#0c081c;font-family:Segoe UI,Arial,sans-serif;">',
        '<div style="max-width:440px;margin:0 auto;background:linear-gradient(170deg,#1a1030,#0e0920);border:1px solid #6a4fc0;border-radius:18px;padding:32px 28px;color:#f1e6ff;">',
        '<p style="margin:0 0 6px;font-size:24px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:#ffb648;">Fasiolas</p>',
        '<p style="margin:0 0 22px;font-size:12px;letter-spacing:2px;text-transform:uppercase;color:#b9a4e6;">Realaus laiko kortu zaidimai</p>',
        '<h1 style="margin:0 0 12px;font-size:19px;color:#f8ecff;">Slaptazodzio atstatymas</h1>',
        '<p style="margin:0 0 22px;font-size:14px;line-height:1.5;color:#c3aeee;">Gavome prasyma atstatyti tavo Fasiolas paskyros slaptazodi. Spausk mygtuka ir nustatyk nauja slaptazodi.</p>',
        `<a href="${resetLink}" style="display:block;text-align:center;background:linear-gradient(180deg,#ffb648,#e07f14);color:#2b1500;font-size:15px;font-weight:800;text-decoration:none;padding:13px 18px;border-radius:12px;">Nustatyti nauja slaptazodi</a>`,
        '<p style="margin:22px 0 6px;font-size:12px;color:#8d7ab5;">Jei mygtukas neveikia, nukopijuok sia nuoroda i narsykle:</p>',
        `<p style="margin:0 0 22px;font-size:12px;word-break:break-all;"><a href="${resetLink}" style="color:#c9a6ff;">${resetLink}</a></p>`,
        '<p style="margin:0;font-size:12px;line-height:1.5;color:#8d7ab5;">Nuoroda galioja 30 minuciu. Jei slaptazodzio atstatyti nepraseisi - ignoruok si laiska, tavo paskyra saugi.</p>',
        "</div>",
        "</div>",
      ].join(""),
    }),
